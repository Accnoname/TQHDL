...
5. [Xử lý & Nội suy dữ liệu](#xử-lý--nội-suy-dữ-liệu)
...
├── 📁 data/
│   └── 📁 raw/
│       └── ai_impact_jobs_2010_2025.csv # Dataset gốc đã được xử lý
├── 📁 src/
...
## Xử lý & Nội suy dữ liệu

Hệ thống tích hợp quy trình tiền xử lý mạnh mẽ, tự động nội suy (Data Imputation) và lấp đầy các khoảng trống thông tin (Missing Data) tuân theo **phân phối xác suất chuẩn**. Thuật toán sẽ xử lý và sinh thêm các bản ghi bổ khuyết một cách logic dựa trên xu hướng tăng trưởng AI thực tế, lạm phát lương và rủi ro tự động hoá theo đặc thù từng ngành nghề. Điều này giúp Dashboard phản ánh bức tranh thị trường với độ chính xác cao nhất mà không bị nhiễu do dữ liệu khuyết.
